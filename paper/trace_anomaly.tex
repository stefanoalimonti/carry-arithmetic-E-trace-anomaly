...
\(D \in \{1, 2\}\). We say the pair \((u, v)\) is \textbf{D-odd} if \(D = 1\),
i.e.,
...
four \textbf{sectors} are \((a_1, c_1) \in \{(0,0), (0,1), (1,0), (1,1)\}\),
with counts \(N_{a_1 c_1}(K)\) among the D-odd pairs.
...
\(\{k/L : k = 0, \ldots, L-1\}\). For general \(L\), a boundary correction of
order \(O(1/L)\) arises. Since \(L = 2K - 1\) is odd for all \(K\), the exact
orthogonality never holds at finite \(K\); the correction is \(O(1/K)\) and
vanishes in the sector-ratio limit \(K \to \infty\).
...
\item
  ...
  the Markov chain on \(\{0, 1, \ldots\}\) started from
